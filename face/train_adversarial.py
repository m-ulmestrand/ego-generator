...
class Discriminator(nn.Module):
    def __init__(self, z_dim: int = 2, device: str = "cuda") -> None:
...
def main():
    ...
    discriminator = Discriminator(device=device, z_dim=z_dim)
    ...
    for epoch in range(n_epochs):
        ...
        for data in tqdm(train_dataset, "Epoch progress"):
            ...
            g_loss = 0.001 * adversarial_loss(discriminator(encoded_imgs), ONES) + \
                     0.999 * reconstruction_loss(decoded_imgs, real_imgs)
            g_loss.backward()
            opt_G.step()
